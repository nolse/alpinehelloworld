        ...
        stage('Build image') {
            steps {
                sh """
                    ...
                    docker build --no-cache --platform linux/amd64 -t alphabalde/${IMAGE_NAME}:${IMAGE_TAG} .
                """
            }
        }
        ...
        stage('Run container') {
            steps {
                sh """
                    ...
                    docker run -d --name ${CONTAINER_NAME} -p 5000:5000 -e PORT=5000 alphabalde/${IMAGE_NAME}:${IMAGE_TAG}
                """
            }
        }
